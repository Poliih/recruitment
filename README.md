...
|   |   |-- home.component.html
|   |-- job-create/
|   |   |-- job-creat.component.html
|   |   |-- job-creat.component.ts
|   |-- job-detail/
...
this.authService.isAuthenticated().subscribe(status => isLoggedIn = status);
...
<div class="container">
  ...
  <form (ngSubmit)="createJob()">
    <div class="form-group">
      ...
      <input type="text" id="title" class="form-control" [(ngModel)]="job.title" name="title" required>
    </div>
    <div class="form-group">
      <label for="description">Descrição:</label>
      <textarea id="description" class="form-control" [(ngModel)]="job.description" name="description" required></textarea>
    </div>
...
<div class="container">
  <h2>{{ job?.title }}</h2>
  <p>{{ job?.description }}</p>
  <button class="btn btn-primary" (click)="apply()">Candidatar-se</button>
</div>
...
<div class="container">
  ...
  <ul class="list-group">
    <li *ngFor="let job of jobs" class="list-group-item">
      <a [routerLink]="['/job', job.id]">{{ job.title }}</a>
    </li>
  </ul>
</div>
...
import { HttpClientModule } from '@angular/common/http'; // Necessário para comunicação com a API
...
<form (ngSubmit)="apply()">
  <div>
    ...
    <input id="name" [(ngModel)]="applicantName" name="name" required>
  </div>
  <div>
    <label for="email">Email:</label>
    <input id="email" [(ngModel)]="applicantEmail" name="email" required>
  </div>
...
<div *ngIf="job">
  <h2>{{ job.title }}</h2>
  <p>{{ job.description }}</p>
  <a [routerLink]="['/apply', job.id]">Apply</a>
</div>
...
import { Component } from '@angular/core';
import { RouterOutlet } from '@angular/router'; // Importação necessária para roteamento em componentes standalone
...
standalone: true,
imports: [RouterOutlet] // Importa RouterOutlet para componentes standalone
})
export class AppComponent {
title = 'my-app';
}
...
import { NgModule } from '@angular/core';
import { BrowserModule } from '@angular/platform-browser';
// Remova importações e declarações do AppComponent se estiver presente
...
providers: [],
bootstrap: [] // Não inclua AppComponent aqui se for standalone
})
export class AppModule { }
...
private apiUrl = 'http://localhost:8080/api/applications'; // Altere para o URL do seu backend
...
export class AuthService {
private apiUrl = 'http://localhost:8080/api'; // Altere para o URL do seu backend
...
export class JobService {
private apiUrl = 'http://localhost:8080/api/jobs'; // Altere para o URL do seu backend
...
<head>
  <meta charset="utf-8">
  <title>RecruitmentFrontend</title>
  <base href="/">
...
ppackage com.example.recruitment.repository;
